...
import { Code, Database, Globe, Layout, Smartphone, Terminal } from "lucide-react"
...
function Feature({ title, text, icon }: FeatureProps) {
  ...
  return (
    ...
    >
      ...
      >
        <Flex w={12} h={12} align="center" justify="center" rounded="md" bg={iconBgColor} color={iconColor} mb={4}>
          {icon}
        </Flex>
  ...
  return (
    <Box py={16} px={{ base: 4, md: 8 }} bg={bgColor}>
      <MotionBox
        ...
        textAlign="center"
      >
        <Heading as="h2" fontSize={{ base: "2xl", md: "3xl" }} fontWeight="bold" mb={3} color={headingColor}>
          Professional{" "}
          <Text as="span" color={accentColor}>
            Expertise
          </Text>
        </Heading>
        <Text fontSize={{ base: "md", lg: "lg" }} color={textColor} maxW="2xl" mx="auto">
          Leveraging industry-standard technologies to deliver robust and scalable solutions
        </Text>
      </MotionBox>

      <SimpleGrid columns={{ base: 1, md: 2, lg: 3 }} spacing={8} mx="auto" maxW="7xl">
        <Feature
